...
3. Separate QCD/Hbb/Hcc centroid and class-specific Mahalanobis diagnostics.
...
6. Class-conditional QCD/Hbb/Hcc Gaussian-mixture likelihood diagnostics.
...
def fit_class_conditional_gaussian_mixture(
    ...
    background subclasses; Wjet is never used during fitting.
    """
...
def plot_score_distribution(
    ...
    xlabel: str,
    path: Path,
    background_label: str = "QCD+Hbb+Hcc (validation)",
    signal_label: str = "Wjet",
) -> None:
...
def main() -> None:
    ...
    combined = {
        "auc_fit_subset_vs_wjet": auc(fit_score, signal_score),
        "auc_heldout_train_vs_wjet": auc(heldout_score, signal_score),
        "auc_validation_vs_wjet": auc(val_score, signal_score),
        ...
        "wjet_scores": score_stats(signal_score),
        "covariance": cov_diag,
    }
    plot_one_roc(
        heldout_score,
        signal_score,
        "QCD+Hbb+Hcc (held-out train)",
        output_dir / "01_mahalanobis_heldout_train_vs_wjet.png",
    )
    plot_one_roc(
        val_score,
        signal_score,
        "QCD+Hbb+Hcc (validation)",
        output_dir / "01_mahalanobis_validation_vs_wjet.png",
    )
    plot_comparison(
        [
    ...
    print(
        f"AUC fit={combined['auc_fit_subset_vs_wjet']:.6f}, "
        f"held-out={combined['auc_heldout_train_vs_wjet']:.6f}, "
        f"validation={combined['auc_validation_vs_wjet']:.6f}"
    )
    ...
    latent_norms = {
        ...
        "wjet": score_stats(signal_full_norm),
    }
    ...
    print(
        ...
        f"Wjet={signal_full_norm.mean():.6f}/{np.median(signal_full_norm):.6f}"
    )
    ...
    local_global_normalized_mse = {
        ...
        "auc_train_vs_wjet": auc(train_lg_nmse, signal_lg_nmse),
        "auc_validation_vs_wjet": auc(val_lg_nmse, signal_lg_nmse),
        "train_background_scores": score_stats(train_lg_nmse),
        "validation_background_scores": score_stats(val_lg_nmse),
        "wjet_scores": score_stats(signal_lg_nmse),
    }
    ...
    plot_one_roc(
        ...
        signal_lg,
        "QCD+Hbb+Hcc (train)",
        output_dir / "03_local_global_mse_train_vs_wjet.png",
    )
    plot_one_roc(
        val_lg,
        signal_lg,
        "QCD+Hbb+Hcc (validation)",
        output_dir / "03_local_global_mse_validation_vs_wjet.png",
    )
    plot_comparison(
    ...
    plot_one_roc(
        val_lg_cos,
        signal_lg_cos,
        "QCD+Hbb+Hcc (validation, cosine distance)",
        output_dir / "09_local_global_cosine_validation_vs_wjet.png",
    )
    plot_score_distribution(
        val_lg_cos,
        signal_lg_cos,
    ...
    plot_one_roc(
        val_lg_nmse,
        signal_lg_nmse,
        "QCD+Hbb+Hcc (validation, unit-normalized MSE)",
        output_dir / "10_local_global_normalized_mse_validation_vs_wjet.png",
    )
    plot_score_distribution(
        val_lg_nmse,
        signal_lg_nmse,
    ...
    print(
        f"Raw MSE AUC train={local_global['auc_train_vs_wjet']:.6f}, "
        f"validation={local_global['auc_validation_vs_wjet']:.6f}"
    )
    print(
        f"Cosine AUC train={local_global_cosine['auc_train_vs_wjet']:.6f}, "
        f"validation={local_global_cosine['auc_validation_vs_wjet']:.6f}"
    )
    print(
        "Normalized-MSE AUC "
        f"train={local_global_normalized_mse['auc_train_vs_wjet']:.6f}, "
        f"validation={local_global_normalized_mse['auc_validation_vs_wjet']:.6f}"
    )
    print(
        ...
        f"Wjet={signal_lg_metrics['global_anchor_norm'].mean():.6f}/"
    ...
    for label in backgrounds:
        ...
        result.update(
            {
                "auc_fit_subset_vs_wjet": auc(class_fit_score, class_signal_score),
                "auc_heldout_train_vs_wjet": auc(
                    class_heldout_score, class_signal_score
                ),
                "auc_validation_vs_wjet": auc(class_val_score, class_signal_score),
                ...
                "wjet_scores": score_stats(class_signal_score),
        ...
        plot_one_roc(
            ...
            output_dir / f"03_{name.lower()}_heldout_train_vs_wjet.png",
        )
        plot_one_roc(
            class_val_score,
            class_signal_score,
            f"{name} (validation)",
            output_dir / f"03_{name.lower()}_validation_vs_wjet.png",
        )
        ...
        print(
            ...
            f"AUC held-out={result['auc_heldout_train_vs_wjet']:.6f}, "
            f"validation={result['auc_validation_vs_wjet']:.6f}"
        )
    ...
    multi_gaussian = {
        ...
        "wjet_scores": score_stats(multi_signal_score),
    ...
    plot_one_roc(
        ...
        output_dir / "04_multi_gaussian_heldout_train_vs_wjet.png",
    )
    plot_one_roc(
        multi_val_score,
        multi_signal_score,
        ...
        output_dir / "04_multi_gaussian_validation_vs_wjet.png",
    )
    plot_comparison(
        [
            (
    ...
    print(
        ...
        f"AUC fit={multi_gaussian['auc_fit_subset_vs_wjet']:.6f}, "
        f"held-out={multi_gaussian['auc_heldout_train_vs_wjet']:.6f}, "
        f"validation={multi_gaussian['auc_validation_vs_wjet']:.6f}"
    )
    ...
    knn_diagnostic = {
        ...
        "auc_fit_subset_vs_wjet": auc(knn_fit_score, knn_signal_score),
        "auc_heldout_train_vs_wjet": auc(
            knn_heldout_score,
            knn_signal_score,
        ),
        "auc_validation_vs_wjet": auc(knn_val_score, knn_signal_score),
        ...
        "wjet_scores": score_stats(knn_signal_score),
    }
    plot_one_roc(
        knn_heldout_score,
        knn_signal_score,
        f"QCD+Hbb+Hcc (held-out train, kNN k={effective_knn_k})",
        output_dir / "05_knn_heldout_train_vs_wjet.png",
    )
    plot_one_roc(
        knn_val_score,
        knn_signal_score,
        f"QCD+Hbb+Hcc (validation, kNN k={effective_knn_k})",
        output_dir / "05_knn_validation_vs_wjet.png",
    )
    plot_comparison(
        [
    ...
    print(
        ...
        f"AUC fit={knn_diagnostic['auc_fit_subset_vs_wjet']:.6f}, "
        f"held-out={knn_diagnostic['auc_heldout_train_vs_wjet']:.6f}, "
    ...
    class_conditional_mixture = {
        ...
        "wjet_scores": score_stats(class_mix_signal_score),
        "fit": class_mix_fit_diagnostics,
    }
    plot_one_roc(
        class_mix_heldout_score,
        class_mix_signal_score,
        "QCD+Hbb+Hcc (held-out train, class-conditional mixture NLL)",
        output_dir / "06_class_conditional_gmm_heldout_train_vs_wjet.png",
    )
    plot_one_roc(
        class_mix_val_score,
        class_mix_signal_score,
        "QCD+Hbb+Hcc (validation, class-conditional mixture NLL)",
        output_dir / "06_class_conditional_gmm_validation_vs_wjet.png",
    )
    plot_comparison(
        [
            (
    ...
    print(
        ...
        f"AUC fit={class_conditional_mixture['auc_fit_subset_vs_wjet']:.6f}, "
        f"held-out={class_conditional_mixture['auc_heldout_train_vs_wjet']:.6f}, "
    ...
    two_sided_local_global = {
        ...
        "wjet_scores": score_stats(lg_two_signal_score),
    }
    plot_one_roc(
        lg_two_heldout_score,
        lg_two_signal_score,
        "QCD+Hbb+Hcc (held-out train, two-sided local-global)",
        output_dir / "07_two_sided_local_global_heldout_train_vs_wjet.png",
    )
    plot_one_roc(
        lg_two_val_score,
        lg_two_signal_score,
        "QCD+Hbb+Hcc (validation, two-sided local-global)",
        output_dir / "07_two_sided_local_global_validation_vs_wjet.png",
    )
    plot_comparison(
        [
            (
    ...
    print(
        "Two-sided local-global: "
        f"AUC fit={two_sided_local_global['auc_fit_subset_vs_wjet']:.6f}, "
        f"held-out={two_sided_local_global['auc_heldout_train_vs_wjet']:.6f}, "
        f"validation={two_sided_local_global['auc_validation_vs_wjet']:.6f}"
    )
    ...
    np.savez_compressed(
        ...
        augmented_global_anchor_norm_wjet=signal_lg_metrics["global_anchor_norm"],
        ...
        augmented_mean_local_view_norm_wjet=signal_lg_metrics["mean_local_view_norm"],
        ...
        multi_gaussian_wjet=multi_signal_score,
        ...
        class_conditional_gmm_wjet=class_mix_signal_score,
        ...
        two_sided_local_global_wjet=lg_two_signal_score,
