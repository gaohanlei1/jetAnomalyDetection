"""
Train the class-token transformer autoencoder for jet anomaly detection.

This script mirrors the existing transformer training entrypoint, but trains a
full reconstruction autoencoder instead of a masked-prediction model. The model
uses eta/phi positional embeddings in the encoder, compresses the event through
the final CLS token, and reconstructs particles from learned decoder slots.
...
python -u scripts/run_train_cls_transformer_ae.py \
    ...
    --weight-decay 1e-4 \
    --loss-type hungarian \
    --output-dir "plots/run-cls-transformer-ae"
"""
...
class TrainClassTokenTransformerAE:
    ...
    def _check_num_slots(self):
        ...
        max_particles = int(multiplicities.max())

        print("Particle multiplicity summary:")
        print(pd.Series(multiplicities).describe(percentiles=[0.5, 0.9, 0.95, 0.99]))

        if max_particles > self.num_slots:
            raise ValueError(
                f"num_slots={self.num_slots} is too small for max particle count {max_particles}. "
                "Increase --num-slots or truncate particles before graph construction."
            )
    ...
    def train(self):
        ...
        self.model = JetClassTokenTransformerAE(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Number of learned decoder output slots. Must cover max particle multiplicity.",
    )
    parser.add_argument(
        "--loss-type",
        choices=["hungarian", "index"],
        default="hungarian",
        help="Reconstruction loss. Hungarian treats particles as a set; index is a debug baseline.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        default="plots/run-cls-transformer-ae",
    ...
    trainer = TrainClassTokenTransformerAE()
    trainer.load()
