...
def plot_property_distribution(datatype1_data: np.ndarray, datatype2_data: np.ndarray, prop_name: str,
    ...
    datatype1_data = [
        item for item in datatype1_data
        if (include_zeros or not math.isclose(item, scaled_zero1, rel_tol=1e-4, abs_tol=0.0))
        and not math.isnan(item)
    ]
    datatype2_data = [
        item for item in datatype2_data
        if (include_zeros or not math.isclose(item, scaled_zero2, rel_tol=1e-4, abs_tol=0.0))
        and not math.isnan(item)
    ]

    ax.set_title(f"{'scaled ' if is_scaled else ''}distribution of {prop_name} {indices_label}{'' if include_zeros else ' (w/o SCALED zeroes)'}")
